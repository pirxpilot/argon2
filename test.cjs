...
const hashes = {
  argon2id: '$argon2id$v=19$m=65536,t=3,p=4$c2FsdHNhbHRzYWx0c2FsdA$rBWULD5jOGpQy32rLvGcmvQMVqIVNAmrCtekWvUA8bw',
  withNull: '$argon2id$v=19$m=65536,t=3,p=4$c2FsdHNhbHRzYWx0c2FsdA$NqchDOxwWbcBzA+0gtsCtyspEQxqKFf4/PO/AoIvo+Q',
  withAd:
  ...
  withSecret: '$argon2id$v=19$m=65536,t=3,p=4$c2FsdHNhbHRzYWx0c2FsdA$8dZyo1MdHgdzBm+VU7+tyW06dUO7B9FyaPImH5ejVOU',
  argon2i: '$argon2i$v=19$m=65536,t=3,p=4$c2FsdHNhbHRzYWx0c2FsdA$1Ccmp7ECb+Rb5XPjqRwEuAjCufY1xQDOJwnHrB+orZ4',
  argon2d: '$argon2d$v=19$m=65536,t=3,p=4$c2FsdHNhbHRzYWx0c2FsdA$VtxJNl5Jr/yZ2UIhvfvL4sGPdDQyGCcy45Cs7rIdFq8',
  rawArgon2id: Buffer.from('ac15942c3e63386a50cb7dab2ef19c9af40c56a2153409ab0ad7a45af500f1bc', 'hex'),
  rawWithNull: Buffer.from('36a7210cec7059b701cc0fb482db02b72b29110c6a2857f8fcf3bf02822fa3e4', 'hex'),
  rawArgon2i: Buffer.from('d42726a7b1026fe45be573e3a91c04b808c2b9f635c500ce2709c7ac1fa8ad9e', 'hex'),
  rawArgon2d: Buffer.from('56dc49365e49affc99d94221bdfbcbe2c18f743432182732e390aceeb21d16af', 'hex'),
...
describe('hash', () => {
  it('hash with argon2i', async () => {
    assert.equal(hashes.argon2i, await argon2.hash(password, { type: argon2i, salt }));
  });

  it('argon2i with raw hash', async () => {
    assert(hashes.rawArgon2i.equals(await argon2.hash(password, { type: argon2i, raw: true, salt })));
  });

  it('hash with argon2d', async () => {
    assert.equal(hashes.argon2d, await argon2.hash(password, { type: argon2d, salt }));
  });

  it('argon2d with raw hash', async () => {
    assert(hashes.rawArgon2d.equals(await argon2.hash(password, { type: argon2d, raw: true, salt })));
  });

  it('hash with argon2id', async () => {
    assert.equal(hashes.argon2id, await argon2.hash(password, { type: argon2id, salt }));
  });

  it('argon2id with raw hash', async () => {
    assert(hashes.rawArgon2id.equals(await argon2.hash(password, { type: argon2id, raw: true, salt })));
  });
  ...
  it('with secret', async () => {
    assert.equal(hashes.withSecret, await argon2.hash(password, { secret, salt }));
  });
});
...
describe('set options', () => {
  ...
  it('hash with high time cost', () => {
    assert.rejects(argon2.hash(password, { timeCost: Number.MAX_SAFE_INTEGER }), RangeError, 'Time cost is too large');
  });
...
describe('verify', () => {
  ...
  it('verify with options without secret', async () => {
    ...
    await assert.doesNotReject(argon2.verify(await argon2.hash(password, { secret }), 'password', {}));
  });
  ...
  it('verify invalid hash function', async () => {
    assert(!(await argon2.verify('$2a$12$R9h/cIPz0gi.URNNX3kh2OPST9/PgBkqquzi.Ss7KIUgO2t0jWMUW', 'abc123xyz')));
  });
});
